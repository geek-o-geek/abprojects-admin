...
export class MapComponent implements OnInit {
  ...
  ngAfterViewInit() {
    this.dt = new Date().toISOString().split("T")[0];
      ...
      const endpoint = `https://cors-everywhere.herokuapp.com/http://istreet-env.eba-hwzzxpcr.us-east-1.elasticbeanstalk.com//attendanceByDate?dt=${new Date().toISOString().split("T")[0]}`;
      const headers = {headers: new HttpHeaders({ "Content-type": "application/json", "Authorization": localStorage.getItem("abprojectsToken") || '' })}
